...
| Check | Evidence |
| --- | --- |
| Guest link parser | `swift test --package-path JazzGuestCore`: 4 tests, 0 failures; manual room fields, scheme/owned-domain links, rejected unsupported hosts and non-HTTPS links. |
...
- Joining a live Jazz meeting, guest admission across organizers, password/lobby behavior and participant rendering. No SDK key or test meeting was available.
...
The next meaningful test is a physical iPhone joined to a representative guest-enabled meeting with an SDK key kept solely in the broker's environment. Use a second participant to verify incoming and outgoing media, initial mute, Leave semantics, and the interruption matrix in [implementation-plan.md](implementation-plan.md).
