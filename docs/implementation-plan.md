**Conference Guest iOS client — implementation plan**
...
Build a native Swift app around the provider's iOS media SDK, retaining its participant rendering and media transport while replacing surrounding UI and controls. Use a small backend to obtain SDK tokens without exposing an SDK key or requiring the participant to log in. Prove current-meeting interoperability and audio-session control on a physical device before committing to the finished client.

**Anonymous website correction (23 September 2026).** The public `salutejazz.ru/calls/create` and guest meeting pages allow account-free creation and joining. This proves an anonymous *first-party web* flow, but not a no-key authorization mode in the public iOS SDK. An embedded website was briefly prototyped and then removed because it cannot establish the requested system-audio reliability. The current app has only the native SDK call path. Without a configured token broker it disables Join rather than presenting a browser as a substitute. Request a supported anonymous native authorization method from the vendor if an SDK project credential is unavailable.
...
**Simulator versus iPhone.** Use the simulator for build, link parsing, UI and lifecycle smoke tests, including sending the app to the background and returning. With a configured test meeting, it can provide preliminary evidence that SDK state remains connected during those transitions. It cannot establish real cellular-call handling, Bluetooth route arbitration, actual camera/microphone behavior, or dependable mixing with X/Instagram audio. Apple's [simulator guidance](https://developer.apple.com/documentation/Xcode/running-your-app-on-simulated-or-physical-devices) says hardware-dependent behavior must be verified on a device; its [audio-session guide](https://developer.apple.com/library/archive/documentation/Audio/Conceptual/AudioSessionProgrammingGuide/OptimizingForDeviceHardware/OptimizingForDeviceHardware.html) specifically excludes reliable interruption, route-change, and cross-app mixing tests in Simulator. Record simulator observations as exploratory and run the acceptance matrix on an iPhone.
