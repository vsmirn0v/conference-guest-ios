...
The signed native app and a browser joined the same meeting. The browser listed the phone as a second participant with microphone and camera off; the device showed both participant tiles and native controls. A longer ordinary background period and a competing nonmixable playback app caused the phone to disappear from the browser participant list; foregrounding restored it. The app process survived. The SDK overwrote `.mixWithOthers` after join; restoring that option did not fix background continuity. Enabling its CallKit feature flag experimentally produced a system call screen stuck at “Connection…”, so that flag is not enabled in the working build. Thus guest join and default mute are demonstrated, while the two crucial audio/lifecycle requirements are **open failures**, not implementation claims.
...
1. **Instrument the real device:** timestamp conference phase, app foreground/background, audio category/mode/options, activation/interruption reasons, actual route, connection state and remote participant membership. Redact invite credentials and participant data. Repeat idle-background and competing-player cases with a second endpoint. Distinguish a suspended process from SDK teardown, socket timeout and media-only pause.
...
3. **Keep legitimate audio active:** `UIBackgroundModes=audio` is already present. Verify actual inbound audio playback with a speaking remote participant when the local mic is off; a silent room may not provide ongoing playback. Use the SDK's supported background-audio mode if available. Do not use silent loops solely to evade iOS suspension.
