...
3. **Keep legitimate audio active:** `UIBackgroundModes` now includes `audio` and `voip` for the genuine outgoing CallKit call. Silent-room membership passed a shorter physical-device test; repeat for 30 minutes and verify speech after silence. Do not use silent loops solely to evade iOS suspension.
...
Release only when the five requested behaviors pass on device, with no crash or unintended media activation. Archive/TestFlight distribution, entitlement setup and cross-organizer policy tests are separate acceptance steps. The present prototype passed shorter background, lock, controlled competing-player and cellular interruption checks; the full acceptance matrix is still open.
...
2. Extend the working CallKit background and interruption recovery to the full soak, real playback apps and hardware route matrix; seek vendor guidance for any SDK-specific failure.
3. Complete the route/PiP decision and Universal Link deployment on an owned domain.
4. Run the full device matrix, fix failures, archive and distribute a test build.

The earlier estimate of roughly 22–33 iOS engineering days plus any vendor-dependent time remains only a planning range. The current signed prototype clears anonymous guest joining and shorter CallKit lifecycle checks; the longer acceptance matrix may alter that estimate. Reimplementing the provider's proprietary signaling/media stack is a separate project, not a small fallback.
